...
```{r}
events_tb_cam <- anns_wide_animals_joined %>% 
  dplyr::filter(species != "Unknown") %>%
  dplyr::mutate(plot_id = stringr::str_sub(id, 1, 4)) %>% 
  dplyr::mutate(date_time = lubridate::ymd_hms(exif_date_time_original,
                                               tz = "Canada/Eastern"),
...
```{r}
clust <- hclust(cluster::daisy(events_tb_cam_hell, "gower"), 
                method = "ward.D2")
plot(clust)
```
...
coords <- cams_sf %>% 
  sf::st_coordinates() %>% 
  as.data.frame()
cams_sf_with_props <- dplyr::bind_cols(cams_sf, coords) %>% 
  sf::st_drop_geometry() %>% 
...
for (pol in 1:3){
  base <- base + 
    ggplot2::geom_path(data = dplyr::filter(pols, L3 == pol),
                       ggplot2::aes(x=X, y=Y),
                       linewidth = 0.2)
}
...
d[1, 4:7] <- d[1, 4:7] * 3
head(d)
ggplot2::ggplot() + scatterpie::geom_scatterpie(ggplot2::aes(x=long, y=lat, group=region), data=d,
                           cols=LETTERS[1:4]) + ggplot2::coord_equal()
```
