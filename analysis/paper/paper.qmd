...
cams_sf <- RoF_CameraDeploymentLog_WLU %>% 
  ...
  dplyr::mutate(is_retrieved = !is.na(date_retrieved))
...
#   ggplot2::theme_bw() +
#   ggplot2::geom_sf() +
#   ggplot2::geom_sf(data=cams_sf)

cams_sf %>%
  ggplot2::ggplot() +
  ggplot2::theme_bw() +
  ggplot2::geom_sf(ggplot2::aes(color = is_retrieved))
```
...
We present a summary of the species identified and an ordination of presence absence data, using proportions of land cover classes observed in a radius of 1 km around each camera location. We used the 2015 Land Cover of Canada dataset.
...
```{r}
#| label: ordination-custom
#| echo: false

source("scripts/3_ordination.R")

custom_rda_plot(test_rda)
```
...
events_tb <- anns_wide_animals_joined %>% 
  dplyr::filter(species != "Unknown") %>%
  ...
  # dplyr::select(camera_id, image_id, species, ymd) %>% 
  dplyr::select(plot_id, species, ymd) %>% 
  # dplyr::group_by(image_id, ind, ymd) %>% 
  # dplyr::summarise(ind_count=dplyr::n())
  dplyr::distinct() %>% 
  dplyr::group_by(plot_id, species) %>%
  dplyr::summarise(day_count=dplyr::n()) %>% 
  dplyr::ungroup() %>% 
  # dplyr::group_by(camera_id) %>% 
  # dplyr::mutate(day_prop = day_count/sum(day_count)) # %>% 
...
events_tb <- events_tb[,colSums(events_tb)>50]
tmp_col <- ncol(events_tb)

# events <- events/rowSums(events)
events <- events_tb %>% 
...
events_tb_cam <- anns_wide_animals_joined_mut %>% 
  # dplyr::mutate(ind = paste(species, sex, age, sep = "_")) %>% 
  # dplyr::select(camera_id, image_id, species, ymd) %>% 
  dplyr::select(camera_id, species, ymd) %>% 
  # dplyr::group_by(image_id, ind, ymd) %>% 
  # dplyr::summarise(ind_count=dplyr::n())
  dplyr::distinct() %>% 
...
# events_tb_cam <- events_tb_cam[,colSums(events_tb_cam)>50]
...
custom_rda_plot(events_rda, sp_scale = 1.5)
```
...
custom_rda_plot(events_rda, sp_scale = 1.5, clust = clust_grp) 
```
...
cams_sf_with_props <- cams_sf_coords %>% 
  ...
  dplyr::distinct() %>% 
  dplyr::ungroup() %>% 
  dplyr::left_join(events, by = "plot_id")
...
```{r}
#| label: ecoregions-ggplot-retrieved
#| echo: false

ggplot2::ggplot(zones_filter) +
  ggplot2::theme_bw() +
  ggplot2::geom_sf() +
  ggplot2::geom_sf(data = cams_sf, cex = 0.5, 
                   ggplot2::aes(col=is_retrieved)) +
  ggplot2::coord_sf(xlim = c(-89, -84),
                    ylim = c(50, 55))
```
...
regions_labels <- zones_filter %>% 
  sf::st_drop_geometry() %>% 
  dplyr::bind_cols(data.frame(x = c(-85, -85, -87.5),
                              y = c(54, 52, 52.7)))
...
base +
  ggplot2::coord_equal(xlim = c(-88, -84.5),
                       ylim = c(50, 54)) +
  ggplot2::geom_label(ggplot2::aes(x=x, y=y, label=ECOREGION), 
                      data = regions_labels) +
  scatterpie::geom_scatterpie(data = cams_sf_with_props,
                              ggplot2::aes(x=X, y=Y, group=plot_id),
                              cols=names(cams_sf_with_props)[4:(4+tmp_col-1)],
...
ggplot2::ggplot(zones_filter) +
  ggplot2::theme_bw() +
  ggplot2::geom_sf() +
  ggplot2::geom_sf(data = cams_sf_clust, cex = 1, 
                   ggplot2::aes(col = grp)) +
  ggplot2::coord_sf(xlim = c(-89, -84),
                    ylim = c(50, 55))
```
...
ggplot2::ggplot(zones_filter) +
  ggplot2::theme_bw() +
  ggplot2::geom_sf() +
  ggplot2::geom_sf(data = cams_sf_geo_clust, cex = 1, 
                   ggplot2::aes(col = grp)) +
  ggplot2::coord_sf(xlim = c(-89, -84),
                    ylim = c(50, 55))
```
...
cams_sf_sp_sum <- cams_sf_sp %>% 
  dplyr::group_by(camera_id, X, Y, species, month) %>% 
...
cams_sf_sp_sum %>% 
  ...
  # dplyr::filter(species == "Caribou", month == 8) %>% 
  ggplot2::ggplot() +
  # ggplot2::geom_hex(ggplot2::aes(x=X, y=Y), bins = 70) +
  # ggplot2::stat_density_2d(ggplot2::aes(fill = ..level..), geom = "polygon", colour="white") +
  # ggplot2::geom_density_2d(ggplot2::aes(x=X, y=Y)) +
  # ggplot2::stat_density_2d(ggplot2::aes(x=X, y=Y, fill = count),
  #                          geom = "raster", contour = T) +
  # ggplot2::stat_density_2d(ggplot2::aes(x=X, y=Y, fill = count),
  #                          geom = "raster", contour = F) +
  # ggplot2::geom_tile(ggplot2::aes(x=X, y=Y, fill = count),interpolate=TRUE) +
  ggplot2::geom_point(ggplot2::aes(x=X, y=Y, size = count)) +
  # ggplot2::scale_fill_continuous(type = "viridis") +
  # ggplot2::scale_fill_distiller(palette=4, direction=1) +
  # ggplot2::theme_bw() + 
