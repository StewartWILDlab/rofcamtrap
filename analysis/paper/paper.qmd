...
We looked at platforms not traditionally used for tagging camera trap images, but used by the larger community of machine learning practitioners for which the task of tagging is a common step, often referred to as labeling. We found two platforms that matched all those criteria: LabelStudio and CVAT. After testing both platforms, LabelStudio was chosen as it was simpler to implement. The Community edition of LabelStudio was deployed on an Ubuntu virtual machine hosted by Compute Canada, using an Apache server and a NGINX reverse proxy, which is a standard approach for custom hosting of websites.
...
We analysed the results of the tagging process under two main forms: presence absence data for each camera; and relative species abundance measured as the number of days, for a given month, that a given species was detected by a given camera.
...
Using the number of days detected as a measure of relative abundance, the most common species (detected more than 10 days in the season) are listed in Table 1 (the full table is available in the appendix at the end of this report). Note the large amount detections in the "bird" and "duck" species categories which could be further distinguished into specific species.
...
sp_tab %>%
  ...
  gt::gt() %>%
  gt::tab_header(
    title = "Relative abundances for species above 10 detection days",
...
As for the spatial distribution of those relative abundances, @fig-map-pies show relative species abundances across the landscape, for species detected more than 50 days in the season (note that the here the detections have grouped by plot number for better visibility, with a plot containing up to 4 cameras) . There are no clear spatial patterns, but some tendencies can be noted: Moose detections are predominant in the northwestern of the sampling array, while caribou and sandhill cranes detections share the most of the detections for the rest of the array. 

```{r}
#| label: fig-map-pies
#| echo: false
#| fig-cap: "Relative species abundances at plot sites (only species species detected more than 50 days in the season are shown)"
#| fig.width: 8
#| fig.height: 6
...
@fig-rda-pa shows the triplot for the RDA on presence-absence data, which yielded a low $R^2$ of `r vegan::RsquareAdj(pa_rda)[2]`, but which is still relevant for exploring species associations. Land cover might not be able to explain community structure, but it can still show which species are found together and around what kind of land cover class. Here, we see that caribou and sandhill cranes are associated with open and treed wetlands (bogs and fens) while black bear and moose are found together in more forested areas. Canada geese are, as is to be expected, associated with water while smaller species like squirrels and martens score away from the larger mammals in a mix of habitats. Note that a full legend of the land cover abbreviations are to be found in the appendix of this report.

```{r}
#| label: fig-rda-pa
#| echo: false
#| fig-cap: "Triplot of RDA on presence-absence data"
...
@fig-rda-events shows the triplot for the RDA on relative abundance data, which  yielded a slightly larger $R^2$ of `r vegan::RsquareAdj(events_rda)[2]`, but reduces the axis scores of the rarer species. We observe similar patterns for caribou, crane, moose, black bear and marten.
...
# plot(clust)
# vegan::cascadeKM(events_gower,2, 5) %>% plot()
```

Clustering allows us to find pattern in the ordination data. @fig-rda-events-clust shows the triplot for the RDA on relative abundance data, with Ward clusters as ellipses. We used the cascading K-means function `cascadeKM` in the `vegan` package to determine the number of relevant clusters to show (in this case, 2 clusters were found to be most relevant. This divides our ordination plot into 2 clusters along the first RDA axis. One cluster for sites primarily found with caribou and cranes, and a second cluster for the rest of the cameras and species. 

```{r}
#| label: fig-rda-events-clust
#| echo: false
#| fig-cap: "Triplot of RDA on relative abundance data, with Ward clusters as ellipses"
...
These clusters can be visualized on a map, which @fig-map-clust shows. There is a slight pattern in which more sites in group 1 are found in the northern part of the array.

```{r}
#| label: fig-map-clust
#| echo: false
#| fig-cap: "Map of clustered camera sites"
...
Spatial clustering allows to add distance between sites as a soft constraint in the clustering algorithm. The extent to which these distances influence the result in balance with the community data can be adjusted. We used a weak effect of `r 0.2` using the `hclustgeo` function in the `ClustGeo` package. @fig-map-clust-geo shows the resulting map of geographically constrained clustered camera sites. The North/South pattern observed below is therein reinforced: camera sites in the northern part of the sampling area are moose likely to detect cranes, caribous and mooses.

```{r}
#| label: fig-map-clust-geo
#| echo: false
#| fig-cap: "Map of geographically constrained clustered camera sites"
...
#| echo: false
#| eval: false
#| fig-cap: "Triplot of RDA on relative abundance data, with geographical Ward clusters as ellipses"
...
mod <- mgcv::gam(count~s(X,Y, bs=""), data = cams_sf_sp_sum_filt)
pred_grid <- 
...
In order to visualize relative abundances through time and space, we can use bubbleplots. The following plots are for caribou and sandhill crane.

![Caribou](../figures/Caribou_bplot.png)
![Sandhill crane](../figures/Sandhill crane_bplot.png)
<!-- ![Moose](../figures/Moose_bplot.png) -->

Alternatively, we can visualize those results as heatmaps. Here is an example for caribou.
...
![Caribou](../figures/Caribou_heatmap.png "Caribou")

## Next steps

Using the tagging information we have produced so far, we were able to show the general patterns that are starting to emerge about our data. The next steps for this project are to complete the tagging process and identify all the detections tagged as unknowns, bird, duck and mustelid species. We will also further explore the community patterns and possibly model abundances using unmarked catch and recapture models. Finally, as mentioned in the introduction, the next major step in this project will be tp use the bounding boxes information we have generated to train an image classifier.

## Acknowledgements

We thank the student volunteers at Wilfrid Laurier University for their help with tagging: [TODO: list volunteers here].
...
lu_dat %>% 
  ...
  gt::gt() %>%
  gt::tab_header(
    title = "Land cover classes abbreviations",
