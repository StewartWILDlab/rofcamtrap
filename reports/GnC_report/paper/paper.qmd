...
p <- base +
  ...
  ggplot2::geom_label(ggplot2::aes(x = x, y = y, label = ECOZONE_NA), inherit.aes = FALSE,
                      size = 2.5, data = regions_labels) +
  ggplot2::geom_point(data = cams_sf_coords, #cex = 1, 
...
ggplot2::ggsave(plot = p, filename = "reports/presentations/rofcamtrap-nov2023/images/camera_array.png", 
                width = 6, height = 8, dpi = 300)
```
...
exif <- readr::read_csv("data/tabular/all_exif.csv") |>
  ...
  dplyr::mutate(source_file = stringr::str_remove(source_file, 
                                               "/workspace/storage/TrailCamStorage_2/")) |>
  dplyr::mutate(source_file = stringr::str_remove(source_file, 
                                               "/workspace/storage/TrailCamStorage/")) |> 
  ...
  dplyr::mutate(image_id = stringr::str_remove(image_id, 
                                               "_workspace_storage_TrailCamStorage_2_")) |>
  dplyr::mutate(image_id = stringr::str_remove(image_id, 
                                               "_workspace_storage_TrailCamStorage_")) |> 
  ...
  dplyr::mutate(
    source_file = 
      stringr::str_remove(source_file, 
                          stringr::fixed("/media/vlucet/TrailCamST/TrailCamStorage/"))) |> 
  dplyr::mutate(
    source_file = 
      stringr::str_remove(source_file, 
                          stringr::fixed("/media/vlucet/TrailCamST/TrailCamStorage_2/")))
  # dplyr::mutate(image_id = stringr::str_sub(image_id, category6))
```
...
saveRDS(anns_wide_noempty, "data/objects/annotations_wide_noempty.rds")
...
# TODO !!!! investigate weird id of cut length 1
